...
class _JobOpportunitiesPageState extends State<JobOpportunitiesPage> {
  ...
  List<ThreadModel> _filterThreads(List<ThreadModel> threads) {
    ...
    if (_selectedFilterOption == 'Full-time' ||
        _selectedFilterOption == 'Part-time' ||
        _selectedFilterOption == 'Remote') {
      filtered = filtered
          .where((t) => t.jobType?.toLowerCase() == _selectedFilterOption.toLowerCase())
          .toList();
    }
    ...
    final created = await Navigator.push<bool>(
      ...
      MaterialPageRoute(
        builder: (_) => CreateThreadForm(
          ...
          roomType:    'job_opportunities',    // أو job_opportunities
          isJobOpportunity: true,              // ← true فى صفحة الوظائف
  ...
  Future<void> _confirmDelete(ThreadModel thread) async {
    ...
    final bool? ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(loc.confirmDeleteThread),      // أضف المفتاح إلى ملفات .arb
    ...
    if (ok == true) {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text(loc.threadDeleted)));
    }
  }
  ...
  Widget _buildFilterChips() {
    ...
    final options = [
      ...
      {'label': loc.fullTime,      'value': 'Full-time'},
      {'label': loc.partTime,      'value': 'Part-time'},
      {'label': loc.remote,        'value': 'Remote'},
  ...
  /// يعالج التبديل فعليًا
  void _onRoomSelected(String value) {
    if (value == 'discussion' && runtimeType != DiscussionRoomPage) {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => DiscussionRoomPage(communityId: widget.communityId)),
      );
    } else if (value == 'advanced' && runtimeType != AdvancedDiscussionRoomPage) {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => AdvancedDiscussionRoomPage(communityId: widget.communityId)),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(AppLocalizations.of(context)!.newJob,  // ← مفتاح ترجمة جديد
