...
class _AdvancedDiscussionRoomPage extends State<AdvancedDiscussionRoomPage> {
  ...
  void initState() {
    super.initState();
    context.read<ThreadBloc>().add(StartRealtimeEvent(widget.communityId, 'discussion_advanced'));
    ...
    _rtSub = RealTimeService.stream().listen(
        ...
        if (roomType != null && roomType != 'discussion_advanced') return;
  ...
  Future<void> _confirmDelete(ThreadModel thread) async {
    ...
    final bool? ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(loc.confirmDeleteThread),
    ...
    if (ok == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(loc.threadDeleted)),
      );
    }
  }
  ...
  Widget _menuRow({
    ...
    return Row(
      children: [
        ...
        if (selected) const Icon(Icons.check, color: AppColors.primaryColor),
      ],
    );
  }
